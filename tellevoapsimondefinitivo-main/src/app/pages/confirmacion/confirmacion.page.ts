...
export class ConfirmacionPage implements OnInit {
  ...
  async checkReserva() {
    ...
    try {
      ...
      this.firebaseSvc.getRealtimeData(reservaPath).subscribe({
        next: async (reservaDoc) => {
          if (reservaDoc) {
            ...
            await this.utils.presentToast({
              message: "No se encuentra ninguna reserva activa",
              duration: 3000,
              color: 'warning',
              position: 'top',
            });
        ...
        error: async (error) => {
          this.reservado = false;
          await this.utils.presentToast({
            message: "Hubo un error al comprobar la reserva. Por favor, intenta de nuevo",
            duration: 3000,
            color: 'danger',
            position: 'top',
          });
      ...
      this.reservado = false;
      await this.utils.presentToast({
        message: "Se ha producido un error inesperado al verificar la reserva",
        duration: 3000,
        color: 'danger',
        position: 'top',
      });
  ...
  async confirmar() {
    ...
    if (this.conductor && this.conductor.uid === userId) {
      await this.utils.presentToast({
        message: "No está permitido reservar para tu propio viaje",
        duration: 5000,
        color: 'primary',
        position: 'top',
    ...
    if ((await Network.getStatus()).connected) {
      if (this.viaje && this.viaje.asientos !== undefined && this.viaje.asientos > 0) {
        ...
        try {
          ...
          await this.utils.presentToast({
            message: "Hubo un error al confirmar la reserva",
            duration: 3000,
            color: 'danger',
            position: 'top',
          });
        ...
        await this.utils.presentToast({
          message: "Los datos del viaje son incorrectos o no hay asientos disponibles",
          duration: 3000,
          color: 'warning',
          position: 'top',
        });
      ...
      await this.utils.presentToast({
        message: "Conexión a internet no disponible",
        duration: 3000,
        color: 'warning',
        position: 'top',
      });
